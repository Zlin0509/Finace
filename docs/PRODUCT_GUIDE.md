...
> 本文已更新至 2026-07-16 的 `0.3.1` Go 版本。第 2 节使用当前 Go 界面实测截图；后续带标注截图来自 `0.2.x` 兼容界面，用于说明尚在迁移中的扩展工作流。截图中的资产、回测和模型参数均为演示数据，历史结果不构成投资建议。
...
产品判断：Go `0.3.1` 使用独立欢迎页作为默认首页；资产页仍保持“总量 -> 结构 -> 明细”的信息顺序。当持仓增加后，建议优先观察集中度和单只基金贡献，而不是只看当日盈亏。
